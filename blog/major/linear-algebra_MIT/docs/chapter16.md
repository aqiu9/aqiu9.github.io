...
上一讲中，我们知道了投影矩阵$$P=A(A^TA)^{-1}A^T$$，Pb将会把向量投影在A的列空间中。
...
在第一个极端情况中，如果$$b\in C(A)$$则有b=Ax。带入投影矩阵$$p=Pb=A(A^TA)^{-1}A^TAx=Ax$$，得证。

在第二个极端情况中，如果$$b\bot C(A)$$则有$$b\in N(A^T)$$，即$$A^Tb=0$$。则$$p=Pb=A(A^TA)^{-1}A^Tb=0$$，得证。

向量b投影后，有b=e+p, p=Pb, e=(I-P)b，这里的p是b在C(A)中的分量，而e是b在$$N(A^T)$$中的分量。
