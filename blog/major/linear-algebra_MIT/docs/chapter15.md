...
从$$\mathbb{R}^2$$空间讲起，有向量$$a, b$$，做b在a上的投影p，如图：
...
从图中我们知道，向量e就像是向量$$b, p$$之间的误差，$$e=b-p, e \bot p$$。p在a上，有$$\underline{p=ax}$$。
...
$$Ax$$总是在A的列空间中，而b却不一定，这是问题所在，所以我们可以将b变为A的列空间中最接近的那个向量，即将无解的$$Ax=b$$变为求有解的$$A\hat{x}=p$$（p是b在A的列空间中的投影，$$\hat{x}$$不再是那个不存在的x，而是最接近的解）。
