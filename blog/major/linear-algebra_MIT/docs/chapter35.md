...
1. *已知$$m\times n$$矩阵A，有$$Ax=\begin{bmatrix}1\\0\\0\end{bmatrix}$$无解；$$Ax=\begin{bmatrix}0\\1\\0\end{bmatrix}$$仅有唯一解，求关于$$m,n,rank(A)$$的信息。*
    ...
    *求证$$A^Ty=c$$至少有一个解*：因为A的列向量线性无关，所以$$A^T$$的行向量线性无关，消元后每行都有主元，且总有自由变量，所以零空间中有非零向量，零空间维数是$$m-r$$（可以直接从$$\dim N\left(A^T\right)=m-r$$得到结论）。
